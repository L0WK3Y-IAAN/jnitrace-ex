...
try:
    ...
    __version__ = _pkg_version("jnitrace-ex")
except Exception:
    __version__ = "0.0.0.dev0"


def _load_jnitrace_script():
    """Load the bundled jnitrace.js, with fallback for development layout."""
    try:
        from importlib_resources import files as _resource_files
        return (_resource_files("jnitrace.build") / "jnitrace.js").read_text(encoding="utf-8")
    except Exception:
        pass
    try:
        from pkg_resources import resource_string
        return resource_string("jnitrace.build", "jnitrace.js").decode("utf-8")
    except Exception:
        pass
    script_path = path.join(path.dirname(__file__), "build", "jnitrace.js")
    if path.isfile(script_path):
        with open(script_path, encoding="utf-8") as f:
            return f.read()
    raise FileNotFoundError(
        "jnitrace.js not found. Run 'npm run build' in the project root, "
        "or install jnitrace from a source distribution."
    )
...
def main():
    ...
    jscode = _load_jnitrace_script().replace("IS_IN_REPL = true", "IS_IN_REPL = false")
